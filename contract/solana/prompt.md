...
### PART4
+ 按照活动发布的类似流程，补全签到信息上链和投票信息上链的代码
...
+ 点击按钮，从签到切换到组队状态时，并没有触发签到信息上链操作

#### 链上错误 0x1773（InvalidPhaseForCheckInUpload）
+ 出现「上链失败 … 0x1773」说明链上活动尚未处于签到阶段，不能执行签到信息上链（upload_check_ins）
+ 正确顺序：发布后链上为 Published → 点击「切换到报名」并用 Phantom 签 start_registration → 点击「切换到签到」并用 Phantom 签 start_check_in → 再点击「切换到组队」签 upload_check_ins
+ 若之前未按顺序做过「报名」「签到」的链上确认，需先回到「已发布」再依次切换到报名、签到、组队，每次用 Phantom 确认
