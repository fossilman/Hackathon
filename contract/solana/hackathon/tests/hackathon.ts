...
describe("hackathon", () => {
  ...
  const DESCRIPTION_HASH = Buffer.alloc(32, 1);

  /** 与链上 seeds 一致：activity_id 以 u64 小端 8 字节传入 */
  function activityPda(
    auth: PublicKey,
  ...
  describe("publish_activity", () => {
    ...
    it("边界: activity_id 使用 u64 大数可正常发布", async () => {
      const bigIdBn = new anchor.BN("9223372036854775807"); // 2^63 - 1，避免 JS number 精度丢失
